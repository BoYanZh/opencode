...
Promise tool executors receive `tool.signal`. Pass it to cancellable work such as `fetch` so stopping the Session also
stops the underlying operation.

```ts
execute: async ({ url }, tool) => {
  const response = await fetch(url, { signal: tool.signal })
